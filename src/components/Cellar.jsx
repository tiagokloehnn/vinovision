...
export default function Cellar({ cellarWines, onSelectWine, onRemoveWine, onScanNew }) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto animate-fadeIn" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)', paddingBottom: 'var(--space-16)' }}>
      ...
      {cellarWines.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3" style={{ gap: 'var(--space-5)' }}>
          {[
            ...
            <div key={label} className="glass-card flex items-center" style={{ padding: 'var(--space-5)', gap: 'var(--space-4)', background: '#FFFFFF' }}>
      ...
      {/* ── ESTADO VAZIO ── */}
      {cellarWines.length === 0 && (
        <div className="glass-card" style={{ padding: 'var(--space-12)', textAlign: 'center', maxWidth: '30rem', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-5)', background: '#FFFFFF' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
          ...
          <div className="flex flex-wrap" style={{ gap: 'var(--space-2)', paddingBottom: 'var(--space-3)', borderBottom: '1px solid var(--border-clean)' }}>
            {FILTERS.map(f => {
              ...
              return (
                <button key={f.id} onClick={() => setFilterType(f.id)} style={{ padding: `var(--space-2) var(--space-4)`, borderRadius: '99px', fontSize: 'var(--text-sm)', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'all 0.2s', background: isActive ? 'var(--wine-primary)' : '#FFFFFF', color: isActive ? '#FFFFFF' : 'var(--text-secondary)', boxShadow: isActive ? '0 4px 12px rgba(114,27,41,0.2)' : '0 2px 6px rgba(0,0,0,0.04)' }}>
                  {f.label}
                </button>
              );
            })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 'var(--space-4)' }}>
            {filtered.map(wine => (
              <div key={wine.id} className="glass-card flex items-center overflow-hidden" style={{ padding: 'var(--space-5)', gap: 'var(--space-5)', background: '#FFFFFF', transition: 'all 0.25s ease' }}
              ...
              >
                {/* Foto */}
                <div onClick={() => onSelectWine(wine)} style={{ width: 'clamp(4.5rem,8vw,6.5rem)', aspectRatio: '3/4', borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.08)', position: 'relative', flexShrink: 0, cursor: 'pointer', background: '#FAF8F5' }}>
                ...
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                  ...
                  <h3 onClick={() => onSelectWine(wine)} style={{ fontFamily: 'var(--font-heading)', fontSize: 'var(--text-lg)', color: 'var(--text-main)', cursor: 'pointer', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.3, fontWeight: 700 }}>
                    {wine.name}
                  </h3>
                ...
                {/* Remover */}
                <button onClick={() => onRemoveWine(wine.id)} style={{ width: 'clamp(2.2rem,3.5vw,2.5rem)', height: 'clamp(2.2rem,3.5vw,2.5rem)', borderRadius: 'var(--radius-md)', background: '#FAF8F5', border: '1px solid var(--border-clean)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'all 0.2s', color: 'var(--text-muted)' }}
